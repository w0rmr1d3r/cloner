...
def get_system_cores() -> int:
    """
    Returns the available system cores or physical cpus by calling the OS to retrieve such information.
    If it cannot be retrieved, it will return -1.
    Not using `len(os.sched_getaffinity(0))` since when it runs, the current process can already be limited to a set
    of cores.
...
def inform_cpu(selected_threads: int):
    ...
    It won't log anything if either is the same number or wasn't able to retrieve the cpu cores.
    """
